...
function save(){
  ...
  saveTimer=setTimeout(()=>{localStorage[KEY]=JSON.stringify(rec)},300);
}
...
function renderSourceStats(records){
  ...
  $('sourceStats').innerHTML=Object.keys(counts).sort().map(key=>`${sourceBadge(key)} <span class="muted">${counts[key]}</span>`).join(' ')||'<span class="muted">暂无</span>';
}
