...
async function aopWikiSearch(){const e=$('aopWikiSearchInput').value.trim();if(!e){$('aopWikiResults').innerHTML='<p class="muted">请输入关键词</p>';return}$('aopWikiResults').innerHTML='<p class="muted">搜索中…</p>';try{const t=await fetchWithTimeout(AOP_WIKI_API+'/api-git/marvinm2/AOPWikiQueries/get-all-aops',{headers:{Accept:'application/json'}},20000);if(!t.ok)throw new Error('API HTTP '+t.status);const n=await t.json();const bindings=n?.results?.bindings||[];const o=e.toLowerCase();const r=bindings.filter(x=>{const title=(x.AOPTitle?.value||'').toLowerCase();const id=(x.AOPID?.value||'').toLowerCase();return title.includes(o)||id.includes(o)}).slice(0,30);if(!r.length){$('aopWikiResults').innerHTML='<p class="muted">未找到与"'+esc(e)+'"相关的AOP<br><small>（已检索 '+bindings.length+' 条AOP记录）</small></p>';return}aopWikiRenderResults(r)}catch(t){$('aopWikiResults').innerHTML='<p style="color:#be123c">搜索失败：'+esc(t.message||'网络错误')+'<br><small>AOP-Wiki API 可能需要科学上网，或API暂时不可用</small></p>'}}
